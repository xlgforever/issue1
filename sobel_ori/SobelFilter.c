...
size_t localThreads[2] = {8, 1};           // {8, 1} is perfect
...
int main(int argc, char** argv) {
    ...
    for(cl_uint i = 0; i < numOfPlatforms; i++ ) {
        ...
        char* cuS0[4095];
        clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, sizeof(cuS0), cuS0, NULL);
        ...
        error = clSetKernelArg(kernel, 0, sizeof(cl_mem),(void*)&inputImageBuffer);
        std::cout<< "_err is " << error <<std::endl;
        printf("clSetKernelArg:");
        error = clSetKernelArg(kernel, 1, sizeof(cl_mem),(void*)&outputImageBuffer);
        std::cout<< "_err is " << error <<std::endl;


        printf("\nclEnqueueNDRangeKernel is start!\n");
        size_t globalThreads[] = {width, height};//{512,512}
        ...
        std::cout << "execution time is:\t" << elapsed << std::endl<< std::endl;
        ...
        //char filename[64] = "InputImage512.bmp";
